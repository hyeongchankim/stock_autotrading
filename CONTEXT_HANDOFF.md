...
28. **텔레그램 알림 구현** (이번 세션): 위 가이드의 "모니터링" 단계에서 알림 수단이 없다는 걸
    ...
    - `main.py`: `daily_max_loss_pct`가 그날 처음 발동하는 순간만 통지(이미 발동해있으면 매
      ...
      보존).
    테스트 8개 추가 (103개 전체 통과). 이 세션엔 `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`가 아직
    설정 안 돼있어서(사용자가 나중에 직접 설정 예정) 실제 텔레그램 메시지 수신까지는 검증 못 함 -
    `send_notification()`이 크리덴셜 없으면 조용히 no-op하는 것만 테스트로 확인됨.
...
TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID                 - 알림용 (이번 세션에 추가, 선택 기능)
                                                         - 아직 이 PC에 설정 안 됨, 사용자가 나중에
                                                           직접 설정 예정 (README "알림" 섹션 참고)
```
KIS 관련은 전부 설정 완료된 상태. Windows 환경변수 변경 후에는 **터미널/앱을 재시작해야**
...
✅ (이어서) 텔레그램 알림 구현 완료 (위 28번 참고) - 진입/청산/청산실패/서킷브레이커발동/크래시
  통지. `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`는 아직 이 PC에 미설정 (사용자가 나중에 직접).
...
- 텔레그램 알림이 실제로 메시지를 보내는 것까지는 검증 안 됨 (크리덴셜 미설정 - `send_notification()`이
  크리덴셜 없을 때 조용히 no-op하는 것만 테스트로 확인됨)
...
남은 건 스케줄러 자연 관찰(수동적, 시간이 필요)과 텔레그램 크리덴셜 설정(사용자 몫) 뿐.

## Git 상태

- 마지막 푸시된 커밋: `c176d42` (텔레그램 알림 구현, 이번 세션)
- 이번 세션 작업은 전부 커밋/푸시됨 - 주요 커밋만 요약: `94bc468`(상태 영속성+사이징 버그),
  ...
  옵트인), `cb1a316`(실전 전환 가이드), `c176d42`(텔레그램 알림) - 각 코드 커밋 사이사이
  핸드오프 문서 갱신 커밋도 별도로 있음 (전체 이력은 `git log --oneline`).
...
1. 사용자가 `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`를 직접 설정하고 실제로 메시지가 오는지
   확인 (위 28번 참고 - Claude가 대신 설정 못 함, README "알림" 섹션에 설정 방법 있음)
