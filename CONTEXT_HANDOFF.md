...
- tr_id 코드(TTTC0802U 등)가 실계좌 매수/매도/POST 호출로 이제 검증됨 (위 15번) - 다만 KIS 공식
  문서 최신본과 재대조는 안 했으니 최종 확인은 권장 (위 22번에서 보듯 여러 출처가 일치해도
  실제로 다를 수 있다는 게 이번에 확인됨)
...
- 마지막 푸시된 커밋: `ca1375b` (체결확인 API 연동, 이번 세션)
- 이번 세션 작업은 전부 커밋/푸시됨 - 주요 커밋만 요약: `94bc468`(상태 영속성+사이징 버그),
  `6dde3e8`(스케줄러), `0e6ce09`(수수료/세금), `e4ff8d2`(호가단위), `b6bfb62`(로그 로테이션),
  `c4fe95e`(BH 슬리브 방향 확정), `ca1375b`(체결확인 API) - 각 코드 커밋 사이사이 핸드오프
  문서 갱신 커밋도 별도로 있음 (전체 이력은 `git log --oneline`). `state.json`과
...
4. tr_id 코드(TTTC0802U/TTTC0801U 등, 주문 관련) 최신 KIS 공식 문서와 재대조 - 체결확인 API에서
   실제로 여러 출처가 일치해도 틀렸던 사례가 나왔으니 (위 22번) 우선순위를 높여도 좋을 듯
