...
26. **슬리피지(백테스트) 옵트인 추가** (이번 세션): 실전 전환 체크리스트를 다 끝낸 뒤 이어서
    ...
    **이걸로 이번 세션 작업 전부 끝남** (실전 전환 체크리스트 완료 + 슬리피지 옵트인 추가).
...
✅ 검증됨:
- 백테스트 로직 전체 (95개 유닛테스트 통과, 이번 세션에 46개 추가됨)
...
남은 건 스케줄러 자연 관찰(수동적, 시간이 필요) 뿐.

## Git 상태

- 마지막 푸시된 커밋: `7fe52d4` (슬리피지 옵트인 추가, 이번 세션)
- 이번 세션 작업은 전부 커밋/푸시됨 - 주요 커밋만 요약: `94bc468`(상태 영속성+사이징 버그),
  ...
  옵트인) - 각 코드 커밋 사이사이 핸드오프 문서 갱신 커밋도 별도로 있음 (전체 이력은
  `git log --oneline`).
...
이번 세션에 진행한 실전 전환 체크리스트 + 슬리피지 옵트인 추가까지 전부 끝났다. 남은 건:
...
3. 정말 실전(`broker.kis.env: real`) 전환을 고려한다면, README "안전장치"/"아직 안 된 것"
   섹션을 처음부터 다시 한번 정독 후 최종 결정
