...
✅ 검증됨:
- 백테스트 로직 전체 (69개 유닛테스트 통과, 이번 세션에 20개 추가됨)
...
- **엔진의 자연 신호를 통한 실주문 체결은 아직 미확인** (지금까지 실행에서 신호가 안 나서 -
  ...
  전체 파이프라인을 통해 실제로 진입하는 건 아직 못 봄)
- 장중 상시 실행을 위한 스케줄러 미구현 (상태 영속성은 이제 준비됨 - main.py를 반복 실행해도
  안전하게 이어짐. 남은 건 몇 분 간격으로 반복 실행시켜줄 스케줄러/cron 뿐)
...
- KIS 실주문 시 실제로 부과되는 수수료/세금이 로컬 현금원장(`kis_cash_ledger`)에는 반영 안 됨
  (MockBroker처럼 commission_pct/sell_tax_pct를 적용하지 않음 - 미미한 드리프트, broker/kis_broker.py
  KisBroker 독스트링 참고)

## Git 상태

- 마지막 푸시된 커밋: `bdc810c` (KIS 실브로커/데이터피드 연동 + 이전 핸드오프 문서, 이전 세션에 커밋됨)
- **이번 세션 작업(상태 영속성 + 사이징 버그 수정 + 주문 체결 검증)은 아직 커밋/푸시 안 됨** -
  변경파일: `.gitignore`, `main.py`, `risk/risk_manager.py`, `broker/kis_broker.py`,
  `CONTEXT_HANDOFF.md` + 신규 `utils/state_store.py`, `portfolio/buy_and_hold.py`,
  `tests/test_state_store.py`, `tests/test_buy_and_hold.py` (+ `tests/test_risk_manager.py`,
  `tests/test_kis_broker.py`에 케이스 추가)
...
1. 이번 세션 변경사항 커밋/푸시
2. 엔진의 자연 신호를 통한 실주문 체결 확인 (스케줄러로 반복 실행하거나, 신호 조건을 잠깐 완화해서
   강제로 한 번 지켜보기)
3. 장중 상시 실행을 위한 스케줄러 구성 (상태 영속성은 이제 준비됨 - 몇 분 간격으로 `python main.py
   --mode paper`를 반복 실행시켜주기만 하면 됨)
