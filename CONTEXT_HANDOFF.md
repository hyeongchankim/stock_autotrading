...
- 마지막 푸시된 커밋: `6dde3e8` (스케줄러 래퍼 스크립트 추가, 이번 세션)
- 이번 세션 작업은 전부 커밋/푸시됨 (`94bc468`: 상태 영속성 + 사이징 버그 수정, `6dde3e8`:
  `run_paper_cycle.bat`). `state.json`과 `StockAutoTradingPaper` 작업 스케줄러 등록은 로컬 머신
...
1. 실전 전환 전 최종 체크리스트 재확인 (README "실제 증권사 API 연동" 섹션의 "아직 안 된 것" 참고)
2. 스케줄러가 며칠 돌면서 자연 신호로 실주문까지 가는지 관찰 (로그: `logs/trading.log`,
   `logs/scheduler_stdout.log`)
3. 호가단위/상한가·하한가/부분체결/슬리피지 반영 여부 검토
