...
❌ 아직 안 됨:
- **엔진의 자연 신호를 통한 실주문 체결 미확인** - 2026-07-15 15:31 기준 재확인: 오늘 스케줄러
  사이클(09:53~15:31, 약 24회) 전부 `positions=[]`, cash 변동 없음 - 여전히 신호가 자연 발생한
  적이 없음(일봉 기반 전략이라 하루 안에서는 신호가 잘 안 바뀜). 신호생성→리스크관리→주문실행
...
2. (선택) `costs.slippage_pct`를 실제 값으로 켜고 재백테스트해서 손절/비중/하이브리드 비율
   결론이 슬리피지 하에서도 여전히 유효한지 확인
3. 정말 실전(`broker.kis.env: real`) 전환을 고려한다면, README "실전 전환 가이드"를 따라
