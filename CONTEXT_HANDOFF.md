...
- 마지막 푸시된 커밋: `c4fe95e` (하이브리드 Buy&Hold 슬리브 처리 방향 확정, 이번 세션)
- 이번 세션 작업은 전부 커밋/푸시됨 (`94bc468`: 상태 영속성 + 사이징 버그 수정, `6dde3e8`:
  ...
  로그 로테이션, `4308238`: 핸드오프 최신화, `c4fe95e`: BH 슬리브 방향 확정). `state.json`과
...
2. 부분체결/슬리피지 반영 여부 검토 (체결내역조회 API 연동 필요)
3. `daily_max_loss_pct` 서킷브레이커 실제 손실 시나리오로 검증
4. tr_id 코드 최신 KIS 공식 문서와 재대조
