...
- `daily_max_loss_pct` 서킷브레이커가 자연 발생 손실로 실제 작동하는 걸 본 적 없음 (유닛
  테스트로만 검증)
- `logs/trading.log`, `logs/scheduler_stdout.log`에 로그 로테이션 없음
- `broker.provider: kis`가 지금 config.yaml에 켜진 상태 - 안 쓸 때는 `mock`으로 되돌리는 게 안전

## Git 상태

- 마지막 푸시된 커밋: `0e6ce09` (KisBroker 로컬 원장에 수수료/세금 반영, 이번 세션)
...
4. 로그 로테이션 추가
5. `daily_max_loss_pct` 서킷브레이커 실제 손실 시나리오로 검증
6. tr_id 코드 최신 KIS 공식 문서와 재대조
