...
- ~~호가단위(tick size) 미반영~~ → 완료: `broker/krx_tick.py`가 KRX의 공개된 고정 호가단위
  ...
  - ~~부분체결/체결확인 안 됨~~ → API 연동은 완료: `KisBroker.get_daily_fills()`가
    ...
    **다만 이 결과를 엔진 로직에 반영하는 건 아직 안 함** - 부분체결 시 리스크매니저가 어떻게
    반응해야 하는지는 별도의 전략적 판단이 필요해서, 지금은 조회 가능한 빌딩블록만 추가된
    상태 (`place_order()`의 `filled=True`는 여전히 "접수됨"만 의미함, 호출부에서 원하면
    `get_daily_fills()`로 직접 대조해야 함). 슬리피지는 여전히 미반영.
