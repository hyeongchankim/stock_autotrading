...
- ~~호가단위(tick size) 미반영~~ → 완료: `broker/krx_tick.py`가 KRX의 공개된 고정 호가단위
  ...
  - ~~부분체결/체결확인 안 됨~~ → API 연동은 완료: `KisBroker.get_daily_fills()`가
    ...
    ~~슬리피지 미반영~~ → 완료: `costs.slippage_pct` 추가 (`commission_pct`/`sell_tax_pct`와
    같은 패턴) - `MockBroker`(백테스트)와 `backtest.benchmark.run_buy_and_hold`(Buy&Hold
    벤치마크, 공정한 비교를 위해 같이 적용)에서 매수는 더 비싸게/매도는 더 싸게 체결되는 걸로
    시뮬레이션. **기본값 0 = 옵트인** - 지금까지 튜닝된 손절 9%/비중 30%/하이브리드 50% 등은
    전부 슬리피지 없이 검증된 값이라, 켜면 그 결론들이 달라질 수 있어서 기존 동작을 조용히
    바꾸지 않도록 사용자가 직접 값을 정하게 함. paper/live(`KisBroker`)엔 적용 안 됨 - 이미
    명시적 가격으로 지정가 주문을 내므로 "슬리피지"가 다른 문제(체결 여부)로 나타남, 이건
    부분체결 수정(위 참고)으로 이미 다룸.
...
**⚠️ 네트워크에 따라 `api.telegram.org`가 차단될 수 있음**: 이 프로젝트 개발 중 실제로 겪은
사례 - 특정 네트워크(가정용 공유기/회사망 등)에서 TCP 연결은 되는데 TLS 핸드셰이크에서
멈추는 걸 확인함 (`curl -v https://api.telegram.org/`로 "Connected" 이후 계속 멈춰있으면
이 패턴 - SNI 기반 필터링으로 추정, `telegram.org` 일반 사이트는 되는데 `api.telegram.org`만
안 되는 게 특징). 이 경우 코드는 정상이지만 **알림이 조용히 하나도 안 간다** - 실패해도 예외를
안 던지는 설계라 트레이딩 자체는 멈추지 않지만, "알림 설정해뒀으니 안심"하고 있다가 정작 필요할
때 못 받을 수 있다. **반드시 스케줄러가 실제로 돌아가는 평소 네트워크에서 알림이 오는지
확인할 것** (모바일 핫스팟 등 다른 네트워크에서 테스트가 성공했다고 해서 평소 네트워크에서도
되는 건 아님). 막혀 있다면: VPN, 다른 네트워크로 상시 전환, 또는 이메일/Windows 토스트 등
다른 알림 채널 병행을 고려.
...
- ~~체결 모델 한계는 동일하게 있음~~ (이 항목을 쓴 시점 기준) - 이후 세션에서 호가단위(KIS
  실주문 대상, `broker/krx_tick.py`)와 슬리피지(백테스트 대상, `costs.slippage_pct`, 기본값
  0=옵트인) 둘 다 추가됨. 수수료·세금은 처음부터 반영됨.
