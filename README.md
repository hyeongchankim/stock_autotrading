...
- ~~호가단위(tick size) 미반영~~ → 완료: `broker/krx_tick.py`가 KRX의 공개된 고정 호가단위
  ...
  - 부분체결/슬리피지는 여전히 미반영 - `place_order`의 `filled=True`는 "KIS가 주문 접수를
    받아줬다"는 뜻이지 "전량 체결됐다"는 보장이 아니다. 실제 체결 여부를 확인하려면 별도로
    체결내역조회 API를 붙여야 하는데, 아직 안 함.
...
- **하이브리드 Buy&Hold 슬리브는 실전에서도 여전히 "가상 계산"일 뿐, 실제 매수가 아니다** - paper/live
  ...
  몫(예: 50만원)뿐이고, 나머지는 그냥 현금으로 남는다. `combined equity` 로그에 찍히는 Buy&Hold
  가치는 어디까지나 비교용 가상 수치이지 실제 계좌 자산이 아니다 - 실전에서 정말 그 절반을
  워치리스트에 분산 투자하고 싶다면 **사용자가 직접 수동으로 매수**해야 한다.
